from gpt_api import AI_trainer
...
def summary_AI (text, summary_type="general",model="text-davinci-003"):
...
@retry_on_openai_error()
def observation_memory_AI(text, notes="", model="text-davinci-003"):
...
@retry_on_openai_error()
def create_question_AI(text,model="text-davinci-003"):
...
@retry_on_openai_error()
def reflection_AI_new(text,reflection_name,model="gpt-4"):
    ...
    return response
@retry_on_openai_error()
def notetaking_AI(text, model="gpt-3.5-turbo-16k"):#needs to get a list of texts
...
@retry_on_openai_error()
def spiky_AI(context, previous_exchanges="", model="gpt-4_8k"):
    """
...
def similarity_approval_AI(question, text, model="text-davinci-003"):
    ...
    template = f"Do you strongly agree or disagree that the question or concept < {question} > is related to this text? Provide a clear 'yes' or 'no' answer along with a strong, opinionated explanation: "
    approval = False
    if isinstance(text, list):
        text = ' '.join(text)
    response = openai.Completion.create(
    ...
    AI_trainer.training_dataset_creation(question + text, response, "similarity_approval_ai_descriptive")
    AI_trainer.training_dataset_creation(response, str(sentiment), "similarity_approval_ai_opinion")
    if sentiment >= 3:
        approval = True
    return approval
...
@retry_on_openai_error()
def goal_approval_AI(text, model="text-davinci-003"):
    template = """Your task is to look through individual exchanges between the client and Spiky to identify those worth documenting. You need to give an opinionated answer. If it is worth it, it must be said. You won't know the project's specifics in advance, but focus on capturing exchanges that:
    Are Relevant: Look for details that seem directly tied to any project's goals or constraints.
    Are Specific: Favor exchanges that offer concrete numbers, timelines, or technologies over vague statements.
    Seem Actionable: Choose information that appears to lead to specific actions or decisions.
    Are Unique: Avoid noting down repetitive or already-known information.
    Use your best judgment to decide if an exchange is important enough to be documented."""
    approval = False
    response = openai.Completion.create(
        model=model,
        prompt=template + text,
    ...
    sentiment = get_sentiment(response)
    AI_trainer.training_dataset_creation(response, str(sentiment), "goal_approval_ai_opinion")
    AI_trainer.training_dataset_creation(text, response + " SENTIMENT: " + str(sentiment), "goal_approval_ai_descriptive")
    if sentiment >= 3:
        approval = True
    return approval
...
@retry_on_openai_error()
def goal_creation_AI(text,model="text-davinci-003"):
...
@retry_on_openai_error()
def create_new_section_AI(text, model="text-davinci-003"):
...
@retry_on_openai_error()
def conversation_comment_AI(text,model="text-davinci-003"):#this function could potentially write better comments by having interactions with the wole document or interact with other AIs
